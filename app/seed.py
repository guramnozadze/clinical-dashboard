...
def seed_user() -> None:
    ...
    if not settings.seed_username or not settings.seed_password:
        logger.info("Seed user not configured, skipping")
        return
    with SessionLocal() as db:
        if get_user_by_username(db, settings.seed_username) is None:
            ...
            logger.info("Seed user %r created", settings.seed_username)
        else:
            logger.info("Seed user %r already exists", settings.seed_username)
